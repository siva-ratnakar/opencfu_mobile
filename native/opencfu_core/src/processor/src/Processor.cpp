...

{
    ...
    if (!fs.isOpened()){
        ...
        fs.open(path, cv::FileStorage::READ);
        if(!fs.isOpened()){
            throw std::runtime_error(std::string("Could not load trained classifier: ") + path);
        }
    }
    ...
    if (!fs.isOpened()){
        ...
        fs.open(path_ps, cv::FileStorage::READ);
        if(!fs.isOpened()){
            throw std::runtime_error(std::string("Could not load post-split trained classifier: ") + path_ps);
        }
    }
