...
void MaskROI::setFromPoints(const std::vector< std::pair<std::vector<cv::Point2f>,int > >& points,int width,int height){
    ...
    for(auto& i : points){
        if(i.second == MASK_TOOL_3P_CIRCLE){
            ...
            // convexHull's output can't alias its input (OpenCV asserts
            // _points.getObj() != _hull.getObj()) -- write into a separate
            // vector, same as the other convexHull call site in Features.cpp.
            std::vector<cv::Point> hull;
            cv::convexHull(pts[0],hull);
            pts[0] = hull;
